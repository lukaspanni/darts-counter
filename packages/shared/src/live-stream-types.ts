...
const liveStreamGameSettingsBaseSchema = z.object({
  startingScore: z.number(),
  outMode: outModeSchema,
  checkoutAssist: z.boolean(),
});

export const liveStreamGameSettingsSchema = z.discriminatedUnion("gameMode", [
  liveStreamGameSettingsBaseSchema.extend({
...
export type LiveStreamGameSettings = z.infer<
  typeof liveStreamGameSettingsSchema
>;

// Game metadata schema - shared between frontend and worker
export const liveStreamGameMetadataSchema = z.object({
  gameId: z.string().uuid(),
  settings: liveStreamGameSettingsSchema,
